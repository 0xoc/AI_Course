...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        return (self.startingPosition, ( )) # pacman state, corners hit
    ...
    def isGoalState(self, state):
        ...
        return self.cornersSet == set(state[1])
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            new_pos = (nextx, nexty)
            new_hit = set(copy.deepcopy(hit))
            
            if new_pos in self.cornersSet:
                new_hit.add(new_pos)
            
            new_hit = tuple(new_hit)

            if not hitsWall:
                successors.append( ( (new_pos, new_hit ), action, 1) )
...
def cornersHeuristic(state, problem):
    ...
    h = 0
    px, py = state[0]   # pacman position

    for c in corners:
        if c in state[1]:
            continue
        cx, cy = c
        hh = abs(cx - px) + abs(cy - py)
        if hh > h:
            h = hh
    return h
